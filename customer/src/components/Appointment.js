...
const Appointment = () => {
  ...
  const initiatePayment = () => {
    ...
    const options = {
      ...
      handler: function (response) {
        ...
        // After payment, call the backend to store appointment data
        storeAppointment(response);
      },
      prefill: {
        name: formData.name,
  ...
  const storeAppointment = async (paymentResponse) => {
    try {
      const appointmentData = {
        ...formData,
        paymentId: paymentResponse.razorpay_payment_id, // Payment ID from Razorpay
        amount: categoryPrices[formData.category]
      };

      // Send the appointment data to the backend
      const response = await fetch("http://localhost:5000/storeAppointment", {
        method: "POST",
        headers: {
      ...
      alert("There was an error storing your appointment. Please try again.");
    }
  };
